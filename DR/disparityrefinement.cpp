...
void DisparityRefinement::Deal_Occlusion(Mat &disparity,const Mat &leftImage, const Mat &plane_map,float** param_plan)
{
	Size disp_size = disparity.size();
	Mat disp_Temp(disp_size,CV_32S);
	
	for( int h = 0; h < disp_size.height; h++)
	{
		for( int w = 0; w < disp_size.width ; w++)
		{
			 int D_l = 0;  //每次在开始处理之前，给D_l赋初始值
			 int tem = disparity.at<int>(h,w);
			if(disparity.at<int>(h,w)>=dMin)
			{
				disp_Temp.at<int>(h,w) = disparity.at<int>(h,w);
			}
			else
			{
				//get the left closest disparity
				 bool condition = true ;
				 int c_w = w ;
				 while(condition)
				 {
					 c_w = c_w + 1;
					 if((c_w+1)>= disp_size.width)
					 {
						 condition = false;
					 }
					 else
					 {
						 if((disparity.at<int>(h,c_w)>=dMin)&&(disparity.at<int>(h,c_w+1)>=dMin))
						 {
							 D_l = (disparity.at<int>(h,c_w) + disparity.at<int>(h,c_w+1))/2;
							 condition = false ; //obtain the left disparity and exit while;
						 }
					 }
				 }//while
				 //经过处理之后，D_l一般有两个值，d和0
				 condition = true ;
				 int c_h ;
				 c_w = w ;
				 vector<int> XX;   //有效区域
				 ...
				 while(condition)
				 {
					...
					else
					{
						...
						h_max = h_max>=disp_size.height? (disp_size.height-1):h_max;   //bingo
						for(int k=h_min;k<=h_max;k++)
						{
							if((disparity.at<int>(k,c_w)>=dMin))
							if((abs(int(disparity.at<int>(k,c_w)-disparity.at<int>(k,c_w-1)))<=1)||(disparity.at<int>(k,c_w)<=D_l))//来自sls算法判断条件
							{
								XX.push_back(c_w);
								YY.push_back(k);
								DD.push_back(disparity.at<int>(k,c_w));
							}
						}
						if(XX.size()>=Tn_l)
						condition = false; 	  //有足够的点了 points are enough
					}
				 }//推出while有两种情况,收集到Tn_l个点和未收集到Tn_l个点
				 
				 bool two_plane;
				 if(XX.size()==0)  //这种情况往往是在最左端的时候
				 {
					disp_Temp.at<int>(h,w) = D_l ;
				 }
				 else
				 {
				 int D_min = *min_element(DD.begin(),DD.end());
				 int D_max = *max_element(DD.begin(),DD.end());
				 if(abs(D_min-D_max)>=2)
					  two_plane = true;
				 else
					  two_plane = false;
				 vector<int> dmin_xx;
				 ...
				 vector<int> dmax_yy;
				 vector<int> dmax_dd;
				 for(vector<int>::size_type i = 0;i<XX.size();i++)
				 {
					if(two_plane)  //区域有两个平面
					{
						if(abs(DD[i]-D_min)<=1)
						{
							dmin_xx.push_back(XX[i]);
							dmin_yy.push_back(YY[i]);
							dmin_dd.push_back(DD[i]);
						}
						if(abs(DD[i]-D_max)<=1)
						{
							dmax_xx.push_back(XX[i]);
							dmax_yy.push_back(YY[i]);
							dmax_dd.push_back(DD[i]);
						}
					}
					else    //如果只有一个平面的话，就把所有数据全部烤过去
					{
							dmin_xx.push_back(XX[i]);
							dmin_yy.push_back(YY[i]);
							dmin_dd.push_back(DD[i]);
				...
				{ 
					int sum_min = 0;
					for(vector<int>::size_type i = 0;i<dmin_xx.size();i++)//计算颜色的相似性
					{
						 sum_min += colorDiff(leftImage.at<Vec3b>(h,w),leftImage.at<Vec3b>(dmin_yy[i],dmin_xx[i]))	;	
					}
					sum_min = int(sum_min/dmin_xx.size());
					int sum_max = 0;
					for(vector<int>::size_type i = 0;i<dmax_xx.size();i++)//计算max地方的颜色相似性
					{
						
						 sum_max += colorDiff(leftImage.at<Vec3b>(h,w),leftImage.at<Vec3b>(dmax_yy[i],dmax_xx[i]));
					}
					sum_max = int(sum_max/dmax_xx.size());
					
					bool which_plane; //true is the d_min, false is the d_max	
					 
					if(sum_min>sum_max)
						which_plane = false;
					else
						which_plane = true ;  //min
					
					if(which_plane)
					{        //d_min planes
						float sum_d = 0 ;
						for(vector<int>::size_type i =0; i<dmin_xx.size();i++)
						{
							float a = param_plan[plane_map.at<uchar>(dmin_yy[i],dmin_xx[i])][0];
							float b = param_plan[plane_map.at<uchar>(dmin_yy[i],dmin_xx[i])][1];
							float c = dmin_dd[i];
							sum_d += a*(w - dmin_xx[i]) + b*(h - dmin_yy[i]) + c;
						}
						disp_Temp.at<int>(h,w) = int(sum_d/dmin_xx.size());	
						if(disp_Temp.at<int>(h,w)<=dMin)
							disp_Temp.at<int>(h,w)=0-DISP_MISMATCH;
						if(disp_Temp.at<int>(h,w)>=dMax)
							disp_Temp.at<int>(h,w) =dMax-1;
					}
					else
					{
						float sum_d = 0 ;
						for(vector<int>::size_type i =0; i<dmax_xx.size();i++)
						{					
							float a = param_plan[plane_map.at<uchar>(dmax_yy[i],dmax_xx[i])][0];
							float b = param_plan[plane_map.at<uchar>(dmax_yy[i],dmax_xx[i])][1];
							float c = dmax_dd[i];
							sum_d += a*(w - dmax_xx[i]) + b*(h - dmax_yy[i]) + c;
						}
						disp_Temp.at<int>(h,w) = int(sum_d/dmax_xx.size());		
						if(disp_Temp.at<int>(h,w)<=dMin)
							disp_Temp.at<int>(h,w)=0-DISP_MISMATCH;
						if(disp_Temp.at<int>(h,w)>=dMax)
							disp_Temp.at<int>(h,w) =dMax-1;
