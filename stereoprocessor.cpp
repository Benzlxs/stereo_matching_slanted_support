...
bool StereoProcessor::init(string &error)
{
	...
	//			temp_costMaps[i][j].create(imgSize, COST_MAP_TYPE);
            }
        }
		uint Tn_l = 16;
		uint Tn_r = 4;
        ...
        aggregation = new Aggregation(images[0], images[1], colorThreshold1, colorThreshold2, maxLength1, maxLength2,dMax,dMin );
        dispRef = new DisparityRefinement(dispTolerance, dMin, dMax, votingThreshold, votingRatioThreshold, maxSearchDepth,
                                          blurKernelSize, cannyThreshold1, cannyThreshold2, cannyKernelSize,Tn_l, Tn_r, S_h);
    }
...
bool StereoProcessor::compute()
{
    if(validParams)
    {
        ...
        subpixelEnhancement();
		/*计算第二次差值的结果*/
		outlierElimination_2();
        regionVoting_2();
        properInterpolation_2();
        discontinuityAdjustment_2();
        subpixelEnhancement_2();
    ...
    for(imageNo = 0; imageNo < 2; ++imageNo)
    {
        for(d = 0; d <= dMax - dMin; d++)
        {
            for(int h = 0; h < imgSize.height; h++)
            {
                ...
                {
			//	float ii = costMaps[imageNo][d].at<costType>(h, w) ;
			//	ii =(float)costMaps[imageNo][d].at<costType>(h, w)/ COST_FACTOR;
			//	float	kk = (float)costMaps[imageNo][d].at<costType>(h, w);
		  //	float  dd = (float)costMaps[imageNo][d].at<costType>(h, w)/ COST_FACTOR;
		    	aggregation->temp_costMaps[imageNo][d].at<float>(h, w) =(float)costMaps[imageNo][d].at<costType>(h, w)/ COST_FACTOR;
				}
			}
		}
	}
	cout << "[StereoProcessor]copy done!" << endl;
    for(imageNo = 0; imageNo < 2; ++imageNo)
    {
       ...
       #pragma omp for schedule(static)
        for(d = 0; d <= dMax - dMin; d++)
        {
            ...
            {
              //  aggregation->aggregation2D(currCostMap, horizontalFirst, imageNo,d);
				(aggregation->bl_adap_coagg(d,imageNo)).copyTo(currCostMap);
...
void StereoProcessor::outlierElimination_2()
{
    cout << "[StereoProcessor] started outlier elimination!" << endl;
...
#ifdef DEBUG
    saveDisparity<int>(disparityMap, "09_dispBoth_oe_2.png");
#endif

    cout << "[StereoProcessor] 2 finished outlier elimination!" << endl;
}
...
void StereoProcessor::regionVoting_2()
{
    vector<Mat> upLimits, downLimits, leftLimits, rightLimits;
...
#ifdef DEBUG
    saveDisparity<int>(disparityMap, "010_dispBoth_rv_2.png");
#endif

    cout << "[StereoProcessor] 2 finished region voting!" << endl;
}

void StereoProcessor::deal_occlusion()
{
	cout<<" hi guy, your algorithm has begin!"<<endl;

	Mat plane_map = aggregation->plane_map;
	float** param_plan = aggregation->param_plan;

	dispRef->Deal_Occlusion(disparityMap, images[0],plane_map,param_plan);

	#ifdef DEBUG
    saveDisparity<int>(disparityMap, "06_dispBoth_pi.png");
	#endif

    cout << "[StereoProcessor] finished deal_occlusion!" << endl;
...
void StereoProcessor::properInterpolation_2()
{
    cout << "[StereoProcessor] started proper interpolation!" << endl;
    dispRef->properInterpolation(disparityMap, images[0]);

#ifdef DEBUG
    saveDisparity<int>(disparityMap, "011_dispBoth_pi_2.png");
#endif

    cout << "[StereoProcessor] 2 finished proper interpolation!" << endl;
}
...
void StereoProcessor::discontinuityAdjustment_2()
{
    cout << "[StereoProcessor] 2 started discontinuity adjustment!" << endl;
    dispRef->discontinuityAdjustment(disparityMap, costMaps);

#ifdef DEBUG
    saveDisparity<int>(disparityMap, "012_dispBoth_da_2.png");
#endif

    cout << "[StereoProcessor] 2 finished discontinuity adjustment!" << endl;
}
...
void StereoProcessor::subpixelEnhancement_2()
{
    cout << "[StereoProcessor] started subpixel enhancement!" << endl;
    floatDisparityMap = dispRef->subpixelEnhancement(disparityMap, costMaps);

#ifdef DEBUG
    saveDisparity<float>(floatDisparityMap, "013_dispBoth_se.png");
#endif

    cout << "[StereoProcessor] 2 finished subpixel enhancement!" << endl;
}
...
void StereoProcessor::saveDisparity(const Mat &disp, string filename, bool stretch)
{
    ...
    for(size_t h = 0; h < imgSize.height; h++)
    {
        for(size_t w = 0; w < imgSize.width; w++)
        {
            ...
            {
                if(stretch)
                disparity =disparity*divide ;//(255 / (max - min)) * (disparity - min);
